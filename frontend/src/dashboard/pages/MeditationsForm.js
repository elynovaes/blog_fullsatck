...
import MeditationSectionPForm from '../components/Forms/MeditationSectionPForm'

const MeditationsForm = ({ data2 }) => {
  ...
  if(section === null) {
    return (
      <div className='w-screen h-screen overflow-x-hidden text-center px-10 py-4 bg-gradient-to-b from-indigo-500 to-indigo-900'>
        ...
        <h1 className='mb-10 text-4xl text-white underline underline-offset-4'>Página Meditação</h1>
        <ul className='flex flex-col w-fit mx-auto gap-3 text-white'>
          <li className='flex items-center justify-center cursor-pointer bg-gradient-to-r from-blue-500 to-indigo-900 transform hover:bg-gradient-to-l hover:scale-110 transition-all duration-300 ease-linear rounded-lg w-96 py-6 px-2 gap-10 text-lg' onClick={() => setSection('MeditationSection')}>
            <div>Seção Meditação</div>
            <span className=''>
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" width="24" height="24" viewBox="0 0 24 24">
                <defs>
                  <path id="right-a" d="M2.29289322,0.292893219 C2.68341751,-0.0976310729 3.31658249,-0.0976310729 3.70710678,0.292893219 C4.09763107,0.683417511 4.09763107,1.31658249 3.70710678,1.70710678 L1.70710678,3.70710678 C1.31658249,4.09763107 0.683417511,4.09763107 0.292893219,3.70710678 C-0.0976310729,3.31658249 -0.0976310729,2.68341751 0.292893219,2.29289322 L2.29289322,0.292893219 Z"/>
  ...
  return (
    <div>
      {section === 'MeditationSection' && <MeditationSectionPForm data2={data2} returned={returned}/>}
    </div>
  )
}

export default MeditationsForm