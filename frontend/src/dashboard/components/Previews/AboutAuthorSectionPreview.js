const AboutAuthorSectionPreview = ({ field }) => {
  const bgSize = {
      ...
      <div className="flex flex-col-reverse lg:flex-row-reverse max-w-screen-xl justify-center items-center gap-16 mx-auto">
        ...
        <div className="max-w-xl">
          ...
          <div className="space-y-2 mb-10">
            <p className="font-extrabold text-3xl text-pink-900">{field.name}</p>
            <p><mark className="bg-pink-900 text-white p-2">{field.prop}</mark></p>
          </div>

          <div className="bg-tertiary w-full hover:bg-pink-600 text-white font-bold rounded">
            <a className="block w-full py-2 px-10 text-center" href="asdfasdfasdf">{field.btn}</a>
          </div>
        </div>

        <div className="relative w-64 h-72  sm:w-96 sm:h-108 mt-10">
          <img className="w-full h-full rounded-3xl absolute z-10" src={field.img} alt="asdfasdf" />
...
export default AboutAuthorSectionPreview